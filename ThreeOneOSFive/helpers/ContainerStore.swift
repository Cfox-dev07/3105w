...
enum ContainerStore {
    ...
    static func resolveApplicationBundlePath(bundleID: String) -> String? {
        guard (try? PatchPathValidator.canonicalBundleIdentifier(bundleID)) == bundleID else {
            return nil
        }
        ...
        for root in applicationBundleRoots {
            ...
            for entry in entries.prefix(4096) {
                let candidate: String
                if root.nested {
                    guard UUID(uuidString: entry) != nil else { continue }
                    let container = (root.path as NSString).appendingPathComponent(entry)
                    let children = (try? fileManager.contentsOfDirectory(atPath: container)) ?? []
                    candidate = children
                        .first(where: { $0.hasSuffix(".app") })
                        .map { (container as NSString).appendingPathComponent($0) } ?? ""
                } else {
                    guard entry.hasSuffix(".app") else { continue }
                    candidate = (root.path as NSString).appendingPathComponent(entry)
                }

                guard !candidate.isEmpty else { continue }
                let infoPath = (candidate as NSString).appendingPathComponent("Info.plist")
